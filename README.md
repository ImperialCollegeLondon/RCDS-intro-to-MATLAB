...
* Or use built-in Copilot or ChatGPT (or similar AIs on dAIsy platform at Imperial)
...
* What do apostrophes do? `x'` `y'`
  * NOTE: `x` and `y` are real matrices. If `x` and `y` are complex matrix, `.'` should be used (Thanks to Yuliang Zhu from EEE). There are more examples can be found [here](https://uk.mathworks.com/help/matlab/ref/double.transpose.html?s_tid=srchtitle_site_search_1_transpose)
...
* Print necessary results with `fprintf`. For example, `fprintf('Minimum ratio was %f on day %i \n', min_ratio, min_day)`. More examples can be found at [fprintf document](https://uk.mathworks.com/help/matlab/ref/fprintf.html?searchHighlight=fprintf&s_tid=srchtitle_support_results_1_fprintf)
